...
const Application = () => {
  ...
  useEffect(() => {
    const fetchApplications = async () => {
      try {
        const token = localStorage.getItem("userToken");
        const res = await fetch("http://localhost:5000/api/user/applications", {
          method: "GET",
    ...
    showConfirm("Bạn có chắc muốn hủy đơn ứng tuyển?", async () => {
      try {
        const res = await fetch(
          "http://localhost:5000/api/user/cancel-application",
          {
            method: "POST",
            headers: {
  ...
  const handleAcceptOffer = (jobId) => {
    ...
    showConfirm(message, async () => {
      try {
        const res = await fetch("http://localhost:5000/api/user/accept-offer", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`,
  ...
  const handleRejectOffer = (jobId) => {
    showConfirm("Bạn chắc chắn muốn từ chối đề nghị?", async () => {
      try {
        const res = await fetch("http://localhost:5000/api/user/reject-offer", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`,
