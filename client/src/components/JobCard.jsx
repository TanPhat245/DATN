...
const JobCard = ({ job }) => {
  ...
  //Handle lưu và bỏ lưu công việc
    ...
    try {
      if (isSaved) {
        ...
        const res = await fetch(
          `http://localhost:5000/api/user/saved-job/${savedJobId}`,
          {
            method: "DELETE",
  ...
  //Kiểm tra và fresh trạng thái đã lưu của công việc
  useEffect(() => {
    const fetchSavedStatus = async () => {
      try {
        const res = await fetch("http://localhost:5000/api/user/saved-jobs", {
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`,
          },
  ...
  //Hiển thị tooltip mô tả công việc khi hover
