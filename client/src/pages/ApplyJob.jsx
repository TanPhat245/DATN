...
const ApplyJob = () => {
        ...
        if (data.success) {
          ...
          if (jobData.recruiter?._id) {
            const res2 = await fetch(
              `http://localhost:5000/api/jobs?recruiterId=${jobData.recruiter._id}`
            );
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      if (!token) return;
      try {
        const resUser = await axios.get("http://localhost:5000/api/user/user", {
  ...
  const handleApply = async () => {
    ...
    try {
      const res = await fetch("http://localhost:5000/api/user/apply", {
        method: "POST",
        headers: {
  ...
  const handleSaveJob = async () => {
    ...
    try {
      const res = await fetch("http://localhost:5000/api/user/save-job", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
    ...
    const checkIfSaved = async () => {
      ...
      try {
        const res = await axios.get(
          `http://localhost:5000/api/user/check-saved/${job._id}`,
          {
            headers: { Authorization: `Bearer ${token}` },
          }
        );
  ...
  //API check đã ứng tuyển
  useEffect(() => {
    const checkIfApplied = async () => {
      if (!token || !job?._id) return;
      try {
        const res = await axios.get(
          `http://localhost:5000/api/user/check-applied/${job._id}`,
          {
            headers: { Authorization: `Bearer ${token}` },
          }
        );
