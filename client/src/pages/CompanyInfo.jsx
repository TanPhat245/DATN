...
const CompanyInfo = () => {
  ...
  const checkFollowStatus = async () => {
    try {
      ...
      const res = await axios.get(
        `http://localhost:5000/api/user/check-follow/${id}`,
        {
  ...
  const handleFollow = async () => {
    try {
      ...
      const res = await axios.put(
        `http://localhost:5000/api/user/follow-recruiter/${id}`,
        {},
        {
          headers: { Authorization: `Bearer ${token}` },
        }
  ...
  useEffect(() => {
    ...
    const fetchJobs = async () => {
      try {
        setLoadingJobs(true);
        const res = await axios.get(
          `http://localhost:5000/api/recruiter/job-recruiter/${id}`
        );
  ...
  const handleSearch = async () => {
    ...
    try {
      const res = await axios.get(
        `http://localhost:5000/api/recruiter/recruiter/${id}/search`,
